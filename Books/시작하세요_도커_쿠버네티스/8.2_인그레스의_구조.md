...
---
You accessed to path "/"
Access Server URL : a206556...ap-northeast-2.elb.amazonaws.com/
Container Hostname : hostname-deployment-6c97ccbc99-wrj9n
Container IP : 192.168.2.153
---
```

# Ingress Controller의 동작 원리 이해

Ingress를 사용하는 방법을 순서대로 정리하면 다음과 같다.

1. 공식 Github에서 제공되는 YAML 파일로 Nginx Ingress Controller를 생성한다.
2. Nginx Ingress Controller를 외부로 노출하기 위한 서비스를 생성한다.
3. 요청 처리 규칙을 정의하는 Ingress Object를 생성한다.
4. Nginx Ingress Controller로 들어온 요청은 Ingress 규칙에 따라 적절한 서비스로 전달한다.

위 과정 중 3번에서 인그레스를 생성하면 Ingress Controller는 자동으로 Ingress를 로드해 Nginx Web Server에 적용한다. 이를 위해 Nginx Ingress Controller는 항상 Ingress 리소스의 상태를 지켜 보고 있으며, 기본적으로 모든 Namespace의 Ingress 리소스를 읽어와 규칙을 적용한다.A

| Kubernetes API에는 특정 Object의 상태가 변화는 것을 확인할 수 있는 Watch라는 API가 있으며, Ingress Controller 또한 Ingress 리소스에 대해 Watch API를 사용한다. Watch는 리소스에서 생성, 삭제, 수정 등의 이벤트가 발생했을 때 이를 알려주는 기능으로, kubectl get 명령어에서도 -w 옵션을 붙여 사용할 수 있다.

```bash
$ kubectl get pods -w # Pod에 변화가 생기면 이를 console에 표시한다.
```

특정 경로와 host name으로 들어온 요청은 Ingress에 정의된 규칙에 따라 Service로 전달된다. 이전에 생성했던 테스트용 Ingress에서는 `/echo-hostname`이라는 경로로 들어온 요청을 `hostname-service`라는 서비스의 80 포트로 전달했다.
...
하지만 요청이 실제로 hostname-service라는 service로 전달되는 것은 아니며, Nginx Ingress Controller는 Service에 의해 생성된 EndPoint로 요청을 직접 전달한다. 즉, 서비스의 ClusterIP가 아닌 EndPoint의 실제 종착 지점들로 요청이 전달되는 셈이다. 이러한 동작을 Kubernetes에서는 bypass라고 부른다. Service를 거치지 않고 Pod로 직접 요청이 전달되기 때문이다.

```bash
$ kubectl get endpoints  # ENDPOINTS 항목에 출력된 지점으로 요청이 전달된다.
NAME                ENDPOINTS                                   AGE
hostname-service    192.168.1.246.5000,192.168.2.156:5000,...   20h
...
```


## Service Bypass의 동작 원리

### 일반적인 Service vs Ingress의 트래픽 흐름

**일반적인 Service 동작**:
```
외부 요청 → Service (ClusterIP) → Pod
```

**Ingress Controller의 bypass 동작**:
```
외부 요청 → Ingress Controller → Pod IP (직접)
                                 ↑ Service ClusterIP를 거치지 않음
```

### Endpoint 확인

Service를 생성하면 Kubernetes가 자동으로 Endpoint 객체를 생성한다. Endpoint는 Service에 연결된 실제 Pod들의 IP 목록이다.

```bash
$ kubectl get endpoints hostname-service
NAME               ENDPOINTS
hostname-service   10.233.86.136:5000,10.233.86.137:5000,10.233.86.138:5000
```

Ingress Controller는 Kubernetes API를 통해 이 Endpoint 목록을 조회하고, Nginx 설정 파일에 Pod IP를 직접 등록한다. 그 결과 요청이 Service의 ClusterIP를 거치지 않고 Pod로 직접 전달된다.

### Bypass의 장점

- **성능 향상**: 네트워크 홉이 1개 줄어들어 더 빠른 응답 속도
- **효율성**: Service의 iptables/IPVS 변환 과정 생략
- **로드밸런싱 제어**: Nginx 자체 로드밸런싱 알고리즘 사용 가능

단, Service 객체는 여전히 필요하다. Ingress Controller가 Endpoint 정보를 얻기 위해 Service를 참조하기 때문이다.
